...
- `POST /abc/upload-from-disk/` - загрузка файла с сервера в MongoDB, загруженные клиентские файлы находятся в `data-tmp`, после успешной загрузки они должны удаляться и обновлять статус.
- `GET /abc/download/{file_id}` - загрузить файл с MongoDB по его внутреннему `file_id`.
